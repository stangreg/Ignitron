...
std::vector<ByteVector> SparkMessage::end_message() {
	...
	for (int this_chunk = 0; this_chunk < num_chunks; this_chunk++) {
		...
		data8.insert(data8.end(), data.begin() + (this_chunk * 0x80),
				data.begin() + (this_chunk * 0x80 + chunk_len));
	...
	ByteVector block_filler = { '\x00', '\x00', '\x00', '\x00', '\x00', '\x00',
			'\x00', '\x00', '\x00' };
...
void SparkMessage::add_prefixed_string(std::string pack_str) {
	...
	std::copy(pack_str.begin(), pack_str.end(),
			std::back_inserter<ByteVector>(byte_pack));
	add_bytes(byte_pack);
}
...
std::vector<ByteVector> SparkMessage::get_current_preset_num() {
	...
	ByteVector msg_vec = { 0x01, 0xfe, 0x00, 0x00, 0x53, 0xfe, 0x17, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf0, 0x01, 0x08, 0x00,
			0x02, 0x10, 0xf7 }; // had an extra 0x79 at the end, likely not needed
...
std::vector<ByteVector> SparkMessage::get_current_preset() {
	// hardcoded message
	std::vector<ByteVector> msg;
	ByteVector msg_vec = { 0x01, 0xfe, 0x00, 0x00, 0x53, 0xfe, 0x3c, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf0, 0x01, 0x0a, 0x01,
...
std::vector<ByteVector> SparkMessage::change_effect_parameter(std::string pedal,
		int param, float val) {
	cmd = '\x01';
	sub_cmd = '\x04';
...
std::vector<ByteVector> SparkMessage::change_effect(std::string pedal1,
		std::string pedal2) {
	cmd = '\x01';
	sub_cmd = '\x06';
...
std::vector<ByteVector> SparkMessage::turn_effect_onoff(std::string pedal,
		boolean enable) {
	cmd = '\x01';
	sub_cmd = '\x15';
