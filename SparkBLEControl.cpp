...
void SparkBLEControl::startServer() {
	...
	uint8_t initialValue[] = {0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,0x77,
	...
	pSparkWriteCharacteristic->setValue(initialValue);
	...
	uint8_t value3[] ={0x01, 0xfe, 0x00, 0x00, 0x53, 0xfe, 0x17, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF0, 0x01, 0x01, 0x00, 0x02, 0x2F, 0xF7};
	uint8_t value[] = {0x01,0xFE,0x00,0x00,0x41,0xFF,0x2C,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x06,0x11,0x4A,0x3E,0x19,0xF7,0xF0,0x01,0x35,0x68,0x03,0x01,0x58,0x0F,0x0E,0x0A,0x19,0x1A,0x07,0x11,0x05,0x4A,0x3F,0x00,0x00,0x00,0x5E,0xF7};
	uint8_t value2[] = {0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,
	...
	pSparkNotificationCharacteristic->setValue(value3);
...
void SparkBLEControl::notifyClients(ByteVector msg){
	...
	if(pSvc) {
		...
		if(pChr) {
			pChr->setValue(static_cast<uint8_t*>(msg.data()),msg.size());
			pChr->notify(true);
		}
	}
}
...
void SparkBLEControl::sendInitialNotification(){
	...
	uint8_t preface1[] ={0x01, 0xFE, 0x00, 0x00, 0x41, 0xFF, 0x1D, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0xF0, 0x01, 0x01, 0x77, 0x03, 0x2F, 0x11, 0x4E, 0x01, 0x04, 0x03, 0x2E, 0xF7};
	uint8_t preface2[] = {0x01, 0xFE, 0x00, 0x00, 0x41, 0xFF, 0x1E, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	...
	uint8_t preface3[] = {0x01, 0xFE, 0x00, 0x00, 0x41, 0xFF, 0x29, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	...
	if(pServer->getConnectedCount()) {
		...
		if(pSvc) {
			NimBLECharacteristic* pChr = pSvc->getCharacteristic(SPARK_BLE_NOTIF_CHAR_UUID);
			if(pChr) {
				//Serial.println("Sending initial notifications");
				pChr->setValue(preface1, 29);
				pChr->notify(true);
				delay(delayValue);
				pChr->setValue(preface2, 30);
				pChr->notify(true);
				delay(delayValue);
				pChr->setValue(preface3, 44);
				pChr->notify(true);
				delay(delayValue);
			} // IF pChr
		} // IF pSvc
